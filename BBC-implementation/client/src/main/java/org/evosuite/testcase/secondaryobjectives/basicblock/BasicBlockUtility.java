...
public class BasicBlockUtility {
    ...
    */
     ...
     */
    private  Set<Integer> detectInterestingCoveredLines(TestChromosome chromosome, BasicBlock semiCoveredBlock, int targetLine) {
        Set<Integer> result = new HashSet<>();
        int lastLine;

        int firstLineInTargetBlock = semiCoveredBlock.getFirstLine();
        int lastLineInTargetBlock = semiCoveredBlock.getLastLine();

        // If target line is in the given semi-covered block, the last line in the block that we need to consider is the target line.
        if(targetLine >= firstLineInTargetBlock && targetLine < lastLineInTargetBlock){
            lastLine = targetLine;
        }else{
            lastLine = lastLineInTargetBlock;
        }

        Set<Integer> coveredLines  = chromosome.getLastExecutionResult().getTrace().getCoveredLines(semiCoveredBlock.getClassName());

        for (Integer line: coveredLines){
            if (line >= firstLineInTargetBlock && line <= lastLine){
                result.add(line);
            }
        }
        if(result.isEmpty()){
            throw new IllegalStateException("The results for interesting lines of a detected semi-covered basic block cannot be empty");
        }
    ...
    /*
        Checks if the given chromosome has covered all of the lines in the given basic block
     */
    private  boolean isFullyCovered(BasicBlock currentBasicBlock, Set<Integer> coveredLines, int targetLine) {
        ...
        int firstLineInTargetBlock = currentBasicBlock.getFirstLine();
        int lastLineInTargetBlock = currentBasicBlock.getLastLine();
        ...
        return coveredLines.contains(lastLine);
    }
    ...
    /*
        Check BBC can be helpfull for the given search objective.
        Returns true if the given search objective is either line coverage or branch coverage.
        */
    ...
    */
    ...
    /*
    Returns the target method from the given search objective
    */
    public String getTargetMethod(FitnessFunction objective) {
        if(objective instanceof org.evosuite.coverage.line.LineCoverageTestFitness ){
    ...
    /*
    Returns the target line number from the given search objective
    */
    public int getTargetLineNumber(FitnessFunction objective){
        if(objective instanceof org.evosuite.coverage.line.LineCoverageTestFitness ){
        ...
        throw new IllegalStateException("BBC only works with line and branch coverage objectives.");
    }
}
