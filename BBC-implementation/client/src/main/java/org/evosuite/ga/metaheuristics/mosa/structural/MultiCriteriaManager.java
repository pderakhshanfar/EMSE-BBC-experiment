					...
					if (instruction.getBasicBlock() != null){
						...
						if (cds.size()==0){
							...
							for (ControlDependency cd : cds) {
								...
								if (Properties.TT && !this.dependencies.containsKey(fitness)){
									this.dependencies.put(fitness,new LinkedHashSet<>());
								}
...
//			BasicBlockMonitor.getInstance().newFFEval(target);
	...
	public void initial_start_times() {
		for (TestFitnessFunction goal : this.currentGoals){
			goal.initial_start_time();
		}
